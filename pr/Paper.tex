...
W celu sprawnienia procesu obliczeñ, ka¿da paczka przesy³ana jest w $m$ czêœciach/fazach ($k = m-1 \ldots 0$). Po skompletowaniu wszystkich czêœci paczek dla danej fazy, nastêpuje przeliczenie dostêpnych danych. Proces powtarza siê $m$ razy, do chwili przeliczenia wszystkich danych.
...
\begin{description}
\item[$packets(j)$] -- zbiór paczek przeznaczonych dla procesora $j$
...
Wartoœci wymienionych funkcji wynikaj¹ bezpoœrednio ze struktury rozwa¿anego systemu przetwarzania.
...
\begin{itemize}
\item wielkoœæ paczki $i$ wraz z paczkami przesy³anymi dalej
$$ S_a(i,k) = a(i,k) + \sum_{p \in next(i)} S_a(p,k)$$
\item czas przesy³ania paczki $i$ (wlicza te¿ czas przesy³ania paczek przesy³anych dalej)
\begin{displaymath}
...
\end{array} \right.
\end{displaymath}
\item czas obliczeñ dla procesora $j$
...
\item czêœæ paczki w danej fazie nie mo¿e byæ przes³ana do kolejnego procesora przed zakoñczeniem przesy³ania czêœci paczek o numerach okreœlonych przez funkcjê $prev(i)$
$$ \forall p \in prev(i) \quad x(p,k)\, r(p,k) + T_c(p,k) \leq r(i,k) \qquad i = 0 \ldots 15, \; k = 0 \ldots m-1$$
\item procesor musi skompletowaæ wszystkie paczki przed rozpoczêciem obliczeñ (dziêki zmiennej binarnej tylko te o niezerowej d³ugoœci)
