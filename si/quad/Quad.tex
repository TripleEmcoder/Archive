...
\subsection{Algorytmy przeszukiwania przestrzeni stanów}
W programie zaimplementowano kilka algorytmów graj¹cych i ich wariantów:
\begin{enumerate}
\item \emph{NegMax} -- najprostszy algorytm minimaksowy, zaimplementowany w celach testowych i porównawczych,
...
W celu zbadania wydajnoœci algorytmów przeprowadzono seriê testów z wykorzystaniem rozgrywek, w których obaj gracze byli sterowani komputerowo. W pary przeciwników po³¹czono algorytmy: \emph{AlfaBeta} i \emph{AlfaBeta} wariant \emph{FailSafe} + \emph{TranspositionTable} oraz \emph{AlfaBeta} wariant \emph{FailSafe} + \emph{TranspositionTable} i \emph{NegaScout}. Wyniki pomiarów przedstawiaj¹ tabele i wykresy 1-4.

\begin{figure}[p]
\centering
\includegraphics[trim=15mm 30mm 20mm 20mm, clip, width=0.9\textwidth]{ab-abfstt-3.pdf}
\caption{Liczba przeszukiwanych wierzcho³ków w kolejnych turach gry algorytmów \emph{AlfaBeta} i \emph{AlfaBeta} wariant \emph{FailSafe} + \emph{TranspositionTable} przy g³êbokoœci 3}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[trim=15mm 30mm 20mm 20mm, clip, width=0.9\textwidth]{ab-abfstt-4.pdf}
\caption{Liczba przeszukiwanych wierzcho³ków w kolejnych turach gry algorytmów \emph{AlfaBeta} i \emph{AlfaBeta} wariant \emph{FailSafe} + \emph{TranspositionTable} przy g³êbokoœci 4}
\end{figure}

Zgodnie z przewidywaniami wprowadzenie wariantu \emph{FailSafe} + \emph{TranspositionTable} daje wyraŸne zmniejszenie iloœci przegl¹danych wierzcho³ków, a co za tym idzie poprawienie wydajnoœci. Jak pokazuj¹ dane z pomiarów, zdarzaj¹ siê przypadki, w których wariant ten wypada nieco gorzej -- wynika to z wykonywania przez niego iteracyjnego pog³êbiania, które mo¿e nieznacznie zwiêkszaæ liczbê przegl¹danych wierzcho³ków. Jest to jednak rzadkie zjawisko.

\begin{figure}[p]
\centering
\includegraphics[trim=15mm 30mm 20mm 20mm, clip, width=0.9\textwidth]{abfstt-ns-3.pdf}
\caption{Liczba przeszukiwanych wierzcho³ków w kolejnych turach gry algorytmów \emph{AlfaBeta} wariant \emph{FailSafe} + \emph{TranspositionTable} i \emph{NegaScout} przy g³êbokoœci 3}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[trim=15mm 30mm 20mm 20mm, clip, width=0.9\textwidth]{abfstt-ns-4.pdf}
\caption{Liczba przeszukiwanych wierzcho³ków w kolejnych turach gry algorytmów \emph{AlfaBeta} wariant \emph{FailSafe} + \emph{TranspositionTable} i \emph{NegaScout} przy g³êbokoœci 4}
\end{figure}

Zgodnie z oczekiwaniami algorytm \emph{NegaScout} daje niewielki, ale stabilny zysk na iloœci przegl¹danych wierzcho³ków w stosunku do algorytmu \emph{AlfaBeta}. Œrednio wynosi on ok. 10\%.

\section{Wnioski, uwagi i podsumowanie}
Szacunkowy rozmiar przestrzeni stanów wynosi $3^{64}$. Rozmiar grafu wynosi oko³o $64^{\textrm{g³êbokoœæ}}$ i nieznacznie maleje wraz z rozwojem rozgrywki. Znacz¹cy spadek rozmiaru grafu obserwuje siê dopiero na kilka ruchów przed zakoñczeniem rozgrywki, po zakoñczeniu fazy ustawiania pionków. 

W³aœnie ze wzglêdu na bardzo du¿y pocz¹tkowy rozmiar grafu niemo¿liwe by³o w rozs¹dnym czasie uzyskanie g³êbokoœci przeszukiwania wiêkszej ni¿ 4, pomimo zastosowania doœæ wyszukanych algorytmów. Najlepsze efekty, zgodnie z przewidywaniami, da³ algorytm \mbox{\emph{NegaScout}} w po³¹czeniu z tablic¹ przejœæ. 
