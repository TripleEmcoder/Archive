...
2. Opis gry
krótka historia (o ile znana!)
dok³adny opis zasad ( je¿eli s¹ jakieœ warianty wskazaæ którego dotyczy implementacja); je¿eli by³y jakieœ w¹tpliwe zasady, opisaæ dok³adnie jakie za³o¿enia poczyniono!
rozmiar i budowa planszy, rodzaje pionów/figur oraz ich kolory, przynale¿noœæ pionów/figur, wszystkie typy ruchów, cel gry, dzia³ania zabronione, dzia³ania przymusowe (np. przymus bicia),
fazy gry (np. faza rozstawiania pionów i faza przesuwania/bicia) a dopuszczalne ruchy
przyk³adowa plansza
3. Implementacja
3.1 Reprezentacja stanu gry
struktury danych reprezentuj¹ce stan w pamiêci komputera (reprezentacja wewnêtrzna)
sposób reprezentacji pionów graczy (obecnych na planszy, zbitych, promocji pionów/figur, pustych pól itp.)
ewentualne konwersje reprezentacji (np. dla potrzeb interfejsu graficznego)
przyk³ad planszy i jej reprezentacja w pamiêci
3.2 Generacja ruchów dopuszczalnych
realizacja ruchów dopuszczalnych zgodnie z zasadami gry dla przyjêtej reprezentacji stanu
sposób implementacji poszczególnych typy ruchów (przesuniêcia, bicia, promocje, zast¹pienia, przechwyty itp.)
kolejnoœæ generowania wszystkich nastêpników danego stanu
...
3.3 Metody przeszukiwania przestrzeni stanów gry - algorytmy graj¹ce
...
4. Testy wydajnoœci i porównania algorytmów
