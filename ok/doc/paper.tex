...
\begin{description}
	\item[$l_i$] -- iloœæ okresów przetwarzania i-tego zadania na I maszynie,
		\begin{description}
		\item[$r'_{ik}$] -- czas rozpoczêcia j-tego okresu przetwarzania i-tego zadania,
		\item[$p'_{ik}$] -- d³ugoœæ j-tego okresu przetwarzania i-tego zadania (dla $k=1,2,\ldots,l_i$);
		\end{description}
...
$$\forall i=1,2,\ldots,n: \forall j=1,2,\ldots,l_i-1: r'_{ij} < r''_{i(j+1)}$$
...
\end{description}
zbiory $H \cup M1$ oraz $M2$ zawieraj¹ parami roz³¹czne przedzia³y, czyli:
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{cmax-tabus-50}
\end{figure}
tabus(step6)
chances(step6)
