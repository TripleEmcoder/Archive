...
\begin{itemize}
\item dla ka¿dego zadania czasy przetwarzania na obu maszynach zawieraj¹ siê w przedziale $\left[ 1, 100\right]$ jednostek:
\begin{displaymath}
...
\item dla ka¿dego zadania czas przygotowania na danej maszynie nie przekracza 50\% czasu przetwarzania na tej maszynie:
\begin{displaymath}
\forall i=1,2,\ldots,n:
...
\item czasy trwania wszystkich okresów niedostêpnoœci zawieraj¹ siê w przedziale  $\left[ 1, 100\right]$ jednostek:
$$\forall j=1,2,\ldots,m: 1 \leq p_j \leq 100$$
\end{itemize}
...
[ca³ka Stieltiesa]
Niech zmienna losowa $X$ odpowiada czasowi przetwarzania na I maszynie, a zmienna losowa $Y$ czasowi przygotowania na I maszynie, a zmienna $Z$ d³ugoœci okresu niedostêpnoœci. 

Zmienna $X$ przyjmuje wartoœci $x_i$ dla $i=2,3,\ldots,100$, gdzie $x_i$ odpowiada czasowi przetwarzania równemu $i$ jednostek czasu. Rozk³ad zmiennej $X$ jest równomierny, zatem:
$$ \textrm{p}(X=x_i) = \frac{1}{99}$$

Wartoœæ œredni¹ zmiennej $X$ mo¿na obliczyæ wg wzoru:

$$ \textrm{E}(X) = \sum _{i=2} ^{100} x_i p(x_i) = 51$$

Zmienna $Y$ przyjmuje wartoœci $y_k$ dla $k=2,3,\ldots,50$, gdzie $y_k$ odpowiada czasowi przetwarzania równemu $k$ jednostek czasu. Rozk³ad zmiennej $Y$ jest równomierny, zatem:

\begin{displaymath}
\textrm{p}(Y=y_k | X=x_i) = \left\{ \begin{array}{ll}
\frac{1}{i/2} & k \leq i/2\\
0 & k>i/2
\end{array} \right.
\end{displaymath}
...
\begin{displaymath}
\textrm{E}(Y) = \int _{-\infty }^{\infty } \textrm{E}(Y|X)\; \textrm{d} F_1(x) = \sum _{i=1} ^{100} \textrm{E}(Y|X=x_i) \textrm{p}(x_i)
\end{displaymath}

Koñcowy wzór ma postaæ:

\begin{displaymath}
\textrm{E}(Y) = \sum _{i=1} ^{100} \sum _{k=1} ^{50} \textrm{p}({y_k}|{x_i}) \textrm{p}(x_i) = 13{,}13
\end{displaymath}

Zmienna $Z$ przyjmuje wartoœci $Z_i$ dla $i=1,2,3,\ldots,100$, gdzie $z_i$ odpowiada czasowi przetwarzania równemu $i$ jednostek czasu. Rozk³ad zmiennej $Z$ jest równomierny, zatem:
...
\begin{description}
	\item[$l_i$] -- iloœæ okresów przetwarzania i-tego zadania na I maszynie, dla $k=1,2,\ldots,l_i$:
		\begin{description}
		\item[$r'_{ik}$] -- czas rozpoczêcia j-tego okresu przetwarzania i-tego zadania,
		\item[$p'_{ik}$] -- d³ugoœæ j-tego okresu przetwarzania i-tego zadania;
		\end{description}
...
$$\forall i=1,2,\ldots,n: \sum_{k=1}^{l_i} p'_{ik} = p'_i$$
...
$$\forall i=1,2,\ldots,n: r'_{il_i} + p'_{il_i} = r''_i$$

\item niemo¿liwe jest przetwarzanie zadania podczas okresu niedostêpnoœci oraz jednoczesne przetwarzanie wiêcej ni¿ jednej czêœci tego samego zadania lub ró¿nych zadañ, innymi s³owy dla zbiorów:
\begin{description}
\item{$H$} -- wszystkich okresów niedostêpnoœci,
\item[$M_1$] -- wszystkich okresów zajêtoœci na I maszynie,
\item[$M_2$] -- wszystkich okresów zajêtoœci na II maszynie,
\end{description}
zbiory $H$ oraz $M_1$ s¹ roz³¹czne, a zbiory $H \cup M1$ oraz $M2$ zawieraj¹ parami roz³¹czne przedzia³y, czyli:
$$H=\left\{ \left[t_1, t_2 \right): 
\exists j=1,2,\ldots,m:
t_1 = r'_{ij} \wedge t_2 = r'_{ij} + p'_{ij} \right\}$$

$$M_1 = \left\{ \left[t_1, t_2 \right): 
\exists i=1,2,\ldots,n:
\exists k=1,2,\ldots,m:
t_1 = s'_i + r'_{ik} \wedge t_2 = r'_{ik} + p'_{ik} \right\}$$

$$M_2 = \left\{ \left[t_1, t_2 \right): 
\exists i=1,2,\ldots,n:
t_1 = s''_i + r''_i \wedge t_2 = r''_i + p''_i \right\}$$

$$\forall a,b \in (H \cup M1): a \cap b = \emptyset$$
$$\forall a,b \in M2: a \cap b  = \emptyset$$
