...
Dok³adne wzory na iloœæ mo¿liwych ruchów w zale¿noœci od liczby zadañ i parametru \emph{distance} wygl¹daj¹ nastêpuj¹co:

\begin{itemize}
\item dla implementacji \emph{SwapTask} i \emph{SwapIndex}:
...
Pierwszym etapem eksperymentu by³ wybór najlepszej z opracowanych wersji algorytmu. W tym celu przeprowadzono 3 serie pomiarów, odpowiednio dla $n=25$, $n=50$ i $n=75$. Testy nie wy³oni³y jednoznacznego zwyciêzcy, dlatego wszystkie wyniki uœredniono do postaci przedstawionej na wykresie na rysunku \ref{fig:impl}. Wykres pokazuje, ¿e dla przypadku œredniego kryterium jakoœci rozwi¹zania nie jest rozstrzygaj¹ce. Wzglêdny b³¹d rozwi¹zañ kszta³tuje siê w poziomie od \SI{3.9}{\%} (wersja \emph{SwapTask-MultiBlock}) do \SI{4.6}{\%} (wersja \emph{MoveIndex-MultiBlock}, zatem ró¿nica w jakoœci jest minimalna. Dlatego zdecydowano siê na wybór wersji najszybszej, która daje dla badanych instancji œrednio ok. 30\% oszczêdnoœæ (czas wykonania na poziomie  od \SI{0.9}{s} -- dla \emph{SwapIndex-DoubleBlock} -- do \SI{1.3}{s} -- dla \emph{MoveIndex-MultiBlock}). Do dalszych rozwa¿añ wybrano zatem algorytm w wersji \emph{SwapIndex-DoubleBlock}, czyli algorytm dokonuj¹cy zamian miejscami zadañ o danych pozycjach, zapamiêtaj¹cy na liœcie tabu wykonany krok i krok do niego odwrotny.
...
W celu znalezienia optymalnych parametrów algorytmu przeprowadzono seriê pomiarów na stosunkowo niewielkich ($n=50$) instancjach ,,próbnych''.
...
\emph{wzrost proporcjonalny do iloœci mo¿liwych ruchów w ka¿dej iteracji, wzory s¹ w moves.nb, mo¿na je wpisaæ tu lub w opisie algorytmu gdzie piszemy o distance} Obrazuje to wykres rysunku \ref{fig:time-distance}
...
Na podstawie wyników pomiarów prêdkoœci dzia³ania algorytmu w nastêpnym kroku zdecydowano przeprowadziæ stosunkowo niewielk¹ iloœæ pomiarów dla parametru \emph{distance}(du¿y koszt czasowy), wiêksz¹ uwagê poœwiêcaj¹c parametrom \emph{tabus} i \emph{chances}.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{frequency-swap}
...
\paragraph{Parametr \emph{chances}}
Rysunek \ref{fig:cmax-chances-50} przedstawia wykres zale¿noœci jakoœci rozwi¹zania od zmieniaj¹cej siê iloœci dodatkowych iteracji. Analiza wykresu prowadzi do nastêpuj¹cych wniosków:
\begin{itemize}
...
\ref{fig:cmax-tabus-75} prowadzi do wniosku, ¿e ów ,,do³ek'', czyli obszar o najmniejszym b³êdzie wzglêdnym, wystêpuje dla $\emph{tabus} \approx \SI{30}{\%}\;\frac{n (n-1)}{2}$, gdzie $\frac{n (n-1)}{2}$ to iloœæ mo¿liwych ruchów.
...
\begin{enumerate}
\item Najlepsza wartoœæ \emph{tabus} wynosi ok. \SI{30}{\%} mo¿liwych ruchów.
