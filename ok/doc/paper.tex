...
\begin{description}
\item[$n$] -- liczba okresów niedostêpnoœci, dla $i=1,2,\ldots,n$:
	\begin{description}
	\item[$r_i$] -- czas rozpoczêcia okresu niedostêpnoœci,
	\item[$p_i$] -- d³ugoœæ okresu niedostêpnoœci;
	\end{description}
\item[$m$] -- liczba szeregowanych zadañ, dla $j=1,2,\ldots,m$:
	\begin{description}
...
$$\sum_{i=1}^n p_i \leq 20\% \sum_{j=1}^m (s'_j + p'_j)$$
...
Jako krok elementarny algorytmu zosta³a wybrana operacja przemieszczenia zadania w inne miejsce w permutacji wyznaczaj¹cej kolejnoœæ ich przetwarzania. W przypadku ogólnym istnieje $(m-1)^2$ mo¿liwych ruchów, a sprawdzenie ka¿dego z nich (obliczenie $C_{max}$ dla powsta³ej permutacji zadañ) wymaga wykonania dalszych $O(m)$ operacji, jak zosta³o to wczeœniej opisane. Daje to z³o¿onoœæ algorytmu rzêdu $O(m^3)$.
...
Dodatkowym elementem algorytmu \emph{tabu search} mo¿e byæ tzw. kryterium aspiracji. Mówi ono, ¿e krok elementarny mo¿e zostaæ wykonany mimo znajdowania siê na liœcie tabu o ile prowadzi on do uzyskania rozwi¹zania o lepszej wartoœci funkcji celu. (\ldots)

\subsubsection{Usprawnienia algorytmu}
\paragraph{Ograniczenie liczby ruchów}
W celu ograniczenia z³o¿onoœci algorytmu wprowadzony zosta³ dodatkowy parametr, zwany dalej \emph{distance}. Mówi on, jaka mo¿e byæ maksymalna odleg³oœæ w permutacji pomiêdzy zadaniem, a miejscem w które zostanie przesuniête w kroku elementarnym. W szczególnoœci, dla odleg³oœci równej 1, rozwa¿ane s¹ jedynie s¹siednie zadania, co daje z³o¿onoœæ algorytmu rzêdu $O(m^2)$.
