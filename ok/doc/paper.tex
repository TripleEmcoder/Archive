...
\begin{displaymath}
\textrm{E}(Y|X) = 
\int_{-\infty}^{\infty} y\; \textrm{d} F(y|x) =
\sum_{k=1}^{50} \textrm{p}({y_k}|{x_i})
\end{displaymath}

\begin{displaymath}
\textrm{E}(Y) =
\int_{-\infty}^{\infty} \textrm{E}(Y|X)\; \textrm{d} F_1(x) =
\sum_{i=1}^{99} \textrm{E}(Y|X=x_i) \textrm{p}(x_i)
\end{displaymath}

Koñcowy wzór ma postaæ:
\begin{displaymath}
\textrm{E}(Y) =
\sum_{i=1}^{99} \sum_{k=1}^{50} \textrm{p}({y_k}|{x_i}) \textrm{p}(x_i) =
\SI{13.13}{}
\end{displaymath}
...
$$\textrm{E}(Z) = \sum_{i=1}^{100} z_i p(z_i) = \SI{50.5}{}$$
...
Wykonanie kroku elementarnego wi¹¿e siê z jego umieszczeniem na liœcie tabu. Pocz¹tkowo listê tabu zaimplementowano jako drzewo binarne. Aby poprawiæ wydajnoœæ opisy kroków s¹ przechowywane w strukturze danych umo¿liwiaj¹cej dodawanie, usuwanie oraz sprawdzanie czy element nale¿y do struktury w czasie sta³ym. W fazie doœwiadczalnej projektu testowane by³y dwie koncepcje listy tabu, zwane dalej \emph{DoubleBlock} i \emph{MultiBlock}, które dla kroku opisanego przez parê liczb $(a, b)$ powoduj¹ zablokowanie:
\begin{description}
...
Aby zwiêkszyæ skutecznoœæ algorytmu wprowadzono dodatkowe kryterium umo¿liwiaj¹ce wykonanie ruchu pomimo, ¿e znajduje siê na liœcie tabu. Jest to mo¿liwe w przypadku je¿eli wykonanie zabronionego ruchu prowadzi do uzyskania rozwi¹zania lepszego ni¿ najlepsze dotychczas znalezione.  
...
Jakoœæ rozwi¹zania mierzono porównuj¹c b³êdy wzglêdne pomiarów, oznaczane dalej \emph{cmax\_error}, przy czym jako wartoœæ dok³adn¹ przyjmowano najmniejsz¹ wartoœæ $C_{max}$ osi¹gniêt¹ w trakcie wykonywania pomiarów dla danej instancji:
\begin{displaymath}
\forall p \in \textrm{P}(\mathbb{I}):
	\emph{cmax\_error}(p) = 
		\frac
			{\emph{cmax}(p)}
			{\min_{q \in \textrm{P}(\mathbb{I})} \emph{cmax}(p)} - 1
\end{displaymath}

W celu zwiêkszenia pewnoœci pomiarów, instancje o zadanych parametrach $n$ i $m$ by³y generowane w 20 egzemplarzach, a pomiary przeprowadzone na nich - odpowiednio uœredniane. Tam, gdzie nie jest to wyszczególnione, iloœæ okresów niedostêpnoœci wynosi³a $m=20\%\,m$. (\emph{dlaczego?})

\subsection{Wybór implementacji}
Pierwszym etapem eksperymentu by³ wybór najlepszej z opracowanych wersji algorytmu. W tym celu przeprowadzono 3 serie pomiarów, odpowiednio dla $n=25$, $n=50$ i $n=75$. Testy nie wy³oni³y jednoznacznego zwyciêzcy, dlatego wszystkie wyniki uœredniono do postaci przedstawionej na wykresie \ref{fig:impl}. Wykres pokazuje, ¿e dla przypadku œredniego kryterium jakoœci rozwi¹zania nie jest rozstrzygaj¹ce. Wzglêdny b³¹d rozwi¹zañ kszta³tuje siê w poziomie od \SI{3.9}{\%} (wersja \emph{SwapTask-MultiBlock}) do \SI{4.6}{\%} (wersja \emph{MoveIndex-MultiBlock}), zatem ró¿nica w jakoœci jest minimalna. Dlatego zdecydowano siê na wybór wersji najszybszej, która daje dla badanych instancji œrednio ok. 30\% oszczêdnoœæ (czas wykonania na poziomie  od \SI{0.9}{s} -- dla \emph{SwapIndex-DoubleBlock} -- do \SI{1.3}{s} -- dla \emph{MoveIndex-MultiBlock}).
...
Wylosowano nastêpuj¹c¹ instancjê problemu:
\begin{verbatim}
2
26 7 54 9 
5
...
Program wywo³ano z parametrami: \emph{distance} = 1, \emph{tabus} = 3, \emph{chances} = 5, wybrano implementacjê \emph{SwapIndex DoubleBlock}.

Po posortowaniu wg czasów przybycia otrzymano nastêpuj¹ce uporz¹dkowanie o funkcji celu równej 135.
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok0}
\end{figure}

Lista tabu jest pusta, chances = 5. Spoœród 4 mo¿liwych ruchów najlepszy jest ruch polegaj¹cy na zamianie miejscami zadañ na pozycjach 4 i 5. Ruch ten prowadzi do nastêpuj¹cego uszeregowania:
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok1}
\end{figure}

Do listy tabu dodano ruch (4,5) i ruch do niego odwrotny (5,4). Dla ruchu polegaj¹cego na zamianie zadañ jest to dok³adnie ten sam ruch, ale w ogólnoœci nie musi tak byæ. Dla implementacji \emph{MoveIndex DoubleBlock} i parametru  \emph{distance} > 1 te ruchy mog¹ byæ ró¿ne. W dalszej czêœci przyk³adu pominiemy ruch odwrotny jako nieistotny w tym przypadku. Ruch nie doprowadzi³ do poprawienia wyniku zatem \emph{chances} zostaje zmniejszone o 1 i wynosi 4.

Spoœród mo¿liwych 3 ruchów najlepszy jest ruch (3,4). Nie znajduje siê on na liœcie tabu, zatem ten ruch zostaje wykonany:
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok2}
\end{figure}

Lista tabu wygl¹da teraz nastêpuj¹co: ( (4,5), (3,4) ). Ruch doprowadzi³ do poprawienia wyniku zatem \emph{chances} pozostaje bez zmian.

Spoœród mo¿liwych 2 ruchów najlepszy jest ruch (1,2). Nie znajduje siê on na liœcie tabu, zatem ten ruch zostaje wykonany:
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok3}
\end{figure}

Lista tabu wygl¹da teraz nastêpuj¹co: ( (4,5), (3,4), (2,3) ). Ruch doprowadzi³ do poprawienia wyniku zatem \emph{chances} pozostaje bez zmian.

Najlepszym ruchem w obecnej sytuacji jest ruch (2,3). Znajduje siê on jednak na liœcie tabu, a jednoczeœnie nie spe³nia kryterium aspiracji (nie prowadzi do poprawienia najlepszego znalezionego rozwi¹zania). Jedynym mo¿liwym ruchem jest (1,2) i musi zostaæ wykonany:
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok4}
\end{figure}

Lista tabu jest pe³na, zatem przed dodaniem do niej nastêpnego ruchu usuwamy ruch dodany najwczeœniej, czyli (4,5). Po dodaniu (1,2) na liœcie tabu znajduj¹ siê nastêpuj¹ce ruchy ( (3,4), (2,3), (1,2) ). Ruch (1,2) doprowadzi³ do pogorszenia rozwi¹zania, wiêc \emph{chances} zostaje obni¿one o 1 i wynosi teraz 3.

Jedynym mo¿liwym ruchem jest (4,5):
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok5}
\end{figure}

Znowu pogorszono rezultat, zatem \emph{chances} wynosi 2. Na liœcie tabu ( (2,3), (1,2), (4,5) ).

Po raz kolejny trzeba wykonaæ jedyny ruch nie znajduj¹cy siê na liœcie tabu, czyli (3,4).
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok6}
\end{figure}

Rozwi¹zanie uleg³o dalszemu pogorszeniu i wynosi 144. Parametr \emph{chances} zosta³ obni¿ony do 1, a lista tabu zaktualizowana: ( (1,2), (4,5), (3,4) ).

Przy aktualnym uporz¹dkowaniu najlepszy jest ruch (1,2). Znajduje siê na liœcie tabu, ale mo¿e byæ wykonany poniewa¿ spe³nia kryterium aspiracji. Prowadzi bowiem do uzyskania rozwi¹zania o $C_{max}$ = 120, które jest lepsze od dotychczasowego najlepszego o wartoœci 133.
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok7}
\end{figure}
Do listy dodajemy w³aœnie wykonany ruch: ( (4,5), (3,4), (1,2) ). Poprawiono rozwi¹zanie, wiêc \emph{chances} pozostaje bez zmian.

Jedyny mo¿liwy ruch (2,3) okazuje siê byæ najlepszym i zostaje wykonany:
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok8}
\end{figure}
Do listy dodajemy w³aœnie wykonany ruch: ( (3,4), (1,2), (2,3) ). Poprawiono rozwi¹zanie, wiêc \emph{chances} pozostaje bez zmian.

Mo¿liwy jest tylko ruch (4,5), a ¿aden z ruchów na liœcie tabu nie prowadzi do poprawienia najlepszego rozwi¹zania. Zatem ruch (4,5) musi byæ wykonany:
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{krok9}
\end{figure}
Ruch ten pogorszy³ funkcjê celu, co powoduje obni¿enie parametru \emph{chances} do 0. To z kolei jest warunkiem zakoñczenia algorytmu. Najlepszym znalezionym rozwi¹zaniem jest uporz¹dkowanie o wartoœci 118 z kroku 7.
...
-d³. tabulisty praktycznie nie wp³ywa, bo wszystkie operacje s¹ wykonywane w czasie O(1)
...
- liniowy wp³yw na d³. dzia³ania, bo algorytm wykona x+chances iteracji. x = iteracje w których poprawia siê wynik

\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{time-distance}
\end{figure}

- wzrost proporcjonalny do iloœci mo¿liwych ruchów w ka¿dej iteracji, wzory s¹ w moves.nb, mo¿na je wpisaæ tu lub w opisie algorytmu gdzie piszemy o distance

\subsubsection{Wp³yw parametrów na jakoœæ rozwi¹zania}
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{cmax-distance-swap}
\end{figure}

- du¿a poprawa mniej wiêcej do distance 20, póŸniej coraz mniejsza, wynika z tego, ¿e dalekie ruch s¹ rzadko wykonywane

- teraz tu trzeba daæ te wykresy czêstoœci wyboru ruchu

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{cmax-chances-50}
\end{figure}

- zwiêkszanie chances daje najlepsze efekty do wartoœci chances <= tabus
- powy¿ej tej wartoœci mo¿e wyst¹piæ dalsza poprawa, ale nie musi, nigdy nie wystêpuje pogorszenie
- dla tej samej wartoœci chances (ten sam czas dzia³ania) wa¿ne jest dobranie tabus
- tabus mo¿e byæ za du¿e (patrz seria 120) lub za ma³e (seria 20) i zwiêkszanie chances daje niewiele
- dla idealnie dobranego tabus (seria 80) nastêpuje sta³a poprawa wyniku
- je¿eli chances < tabus, to tabulista czêsto nie zd¹¿y siê wype³niæ przed przerwaniem algorytmu i dzia³a wtedy tak jakby mia³a d³. chances a nie tabus, szczególnie widoczne dla du¿ych tabus, np. 50, 80 i 120. Tabus 50 przeszkadza dalej ulepszaæ rozwi¹zanie, bo pozwala wróciæ do wczeœniejszych, tabus 120 blokuje zbyt wiele ruchów, tabus 80 jest idealnie

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{cmax-tabus-50}
\end{figure}

- zwiêkszanie tabus powy¿ej poziomu chances nie ma sensu, tabulista i tak nie zd¹¿y siê wype³niæ, o tym by³o trochê w poprzednim punkcie
- najlepsza wartoœæ tabus ujawnia siê dopiero dla chances > tabus (dla serii chances = 80 nie ma dziury, dla 120 jest wyraŸna)


mo¿na tu daæ jeszcze wykresy dla n=75 i pokazaæ ¿e s¹ podobne, czyli to nie przypadek


Ogólne wnioski dla SwapIndex:
- najlepsza wartoœæ tabus to ok. 30\% mo¿liwych ruchów
- najlepsza wartoœæ chances to na pewno wiêcej ni¿ tabus, im wiêcej tym lepiej, ale uwaga na czas
