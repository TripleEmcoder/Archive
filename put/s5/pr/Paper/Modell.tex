...
\item[pakiet $(i,k)$] -- pojedyncza transmisja danych pomiêdzy procesorami, s¹ to dane przesy³ane do procesora docelowego dla kana³u $i$ w~fazie $k$; pakiet zawiera paczkê $(i,k)$ oraz paczki $(p: p \in after(i), k)$;
\item[paczka $(i,k)$] -- fragment pakietu zawieraj¹cy dane do przetworzenia  lokalnego w~fazie $k$ na procesorze docelowym kana³u $i$.
\end{description}

Opisane wy¿ej pojêcia obrazuje fragment diagramu Gantta dla wybranego procesora przedstawiony na rysunku \ref{fig:ModellExample}.
...
\hline
\end{tabular}
\caption{Wartoœci funkcji $channels$}
\label{tab:channels}
\end{table}
...
\hline
\end{tabular}
\caption{Wartoœci funkcji $before$}
\label{tab:before}
\end{table}
...
\hline
\end{tabular}
\caption{Wartoœci funkcji $after$}
\label{tab:after}
\end{table}
...
\item[$S(i)$] -- czas rozruchu ³¹cza dla przesy³ania pakietów kana³u $i$
...
\item[$x(i,k)$] -- zmienna binarna okreœlaj¹ca czy pakiet $(i,k)$ jest wysy³any
...
\item je¿eli pakiet $(i,k)$ ma niezerow¹ d³ugoœæ, to odpowiadaj¹ca mu zmienna binarna musi mieæ wartoœæ 1
